...
void main() {
  group('Trick', () {
    ...
    setUp(() {
      alice = Player(id: 1, name: 'Alice', teamId: 1);
      bob = Player(id: 2, name: 'Bob', teamId: 2);
      charlie = Player(id: 3, name: 'Charlie', teamId: 1);
      dave = Player(id: 4, name: 'Dave', teamId: 2);
    });

    test('addPlay records a card played by a player', () {
      final trick = Trick();
      final card = Card29(Suit.hearts, Rank.nine);
    ...
    test(
      'determineWinner returns highest card of leading suit if no trump',
      () {
        final trick = Trick();
        final card1 = Card29(Suit.hearts, Rank.nine);
    ...
    test('determineWinner returns trump card over higher non-trump', () {
      final trick = Trick();
      final card1 = Card29(Suit.hearts, Rank.ace); // high non-trump
      final card2 = Card29(Suit.spades, Rank.nine); // trump
    ...
    test('determineWinner returns highest trump if multiple trumps played', () {
      final trick = Trick();
      final card1 = Card29(Suit.spades, Rank.nine);
    ...
    test('totalPoints sums the values of all cards in the trick', () {
      final trick = Trick();
      final card1 = Card29(Suit.hearts, Rank.jack); // worth points
      final card2 = Card29(Suit.spades, Rank.nine); // worth points
